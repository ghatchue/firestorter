import { collection, getFirestore, query as firestoreQuery, where } from 'firebase/firestore';
import { runInAction } from 'mobx';

import { Mode } from '../src';
import { Collection, observable } from './init';
...
test('constructor - ref', () => {
  const ref = collection(getFirestore(), 'artists');
  const query = firestoreQuery(ref, where('genre', '==', 'punk'));
...
test('constructor - function', () => {
  const query = (ref) => firestoreQuery(ref, where('genre', '==', 'punk'));
...
test('constructor - expect query function to be called once', () => {
  ...
  const query = (ref) => {
    callCount++;
    return firestoreQuery(ref, where('genre', '==', 'punk'));
  };
  const col = new Collection('artists', {
    query,
  });
...
test('fetch', async () => {
  expect.assertions(2);
  const col = new Collection('artists', {
    query: (ref) => firestoreQuery(ref, where('genre', '==', 'punk')),
  });
...
test('re-fetch', async () => {
  expect.assertions(5);
  const col = new Collection('artists');
  col.query = firestoreQuery(col.ref, where('genre', '==', 'punk'));
  expect(col.docs.length).toBe(0);
  await col.fetch();
  expect(col.docs.length).toBe(1);
  expect(col.docs[0].id).toBe('TheOffspring');
  col.query = (ref) => firestoreQuery(ref, where('genre', '==', 'rock'));
  await col.fetch();
  expect(col.docs.length).toBe(1);
...
test('observable', async () => {
  ...
  const col = new Collection('artists');
  col.query = (ref) => firestoreQuery(ref, where('genre', '==', genre.get()));
  await col.fetch();
  expect(col.docs[0].id).toBe('TheOffspring');
